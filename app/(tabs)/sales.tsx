...
export default function SalesScreen() {
  ...
  const filtered = useMemo(() =>
    sales.filter(s => !search || s.itemName.toLowerCase().includes(search.toLowerCase()) || s.category.toLowerCase().includes(search.toLowerCase()))
      .sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()),
    [sales, search]
  );

  const totalRevenue = useMemo(() => sales.reduce((sum, s) => sum + parseFloat(s.price || '0'), 0), [sales]);
  ...
  return (
    <View style={[styles.root, { backgroundColor: C.background }]}>
      ...
      <Header
        ...
        rightAction={{ icon: 'add', onPress: () => router.push('/sale/add') }}
      />

      {/* Revenue Summary */}
      <View style={[styles.summary, { backgroundColor: C.primaryDark }]}>
        <Text style={styles.summaryLabel}>{t.totalRevenue}</Text>
        <Text style={styles.summaryAmount}>{totalRevenue.toLocaleString()} MMK</Text>
        <Text style={styles.summaryCount}>{sales.length} sale records</Text>
      </View>
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <SaleCard item={item} onPress={() => router.push(`/sale/${item.id}`)} isDark={settings.theme === 'dark'} type="sale" />
        )}
        contentContainerStyle={styles.list}
        showsVerticalScrollIndicator={false}
        ListEmptyComponent={
          <View style={styles.empty}>
            <MaterialIcons name="point-of-sale" size={52} color={C.textMuted} />
            <Text style={[styles.emptyText, { color: C.textMuted }]}>{t.noSales}</Text>
          </View>
        }
      />
      <TouchableOpacity style={[styles.fab, { backgroundColor: C.primary }]} onPress={() => router.push('/sale/add')}>
